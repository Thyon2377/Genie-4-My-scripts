...
action (waiting) var player $1 ; var disarmit $2 when (\w+) offers you.+\b(coffer|trunk|chest|strongbox|skippet|caddy|crate|casket|box)\b
...
container_Wait:
	...
	waitforre offers you a .* (coffer|trunk|(?<!(tea) )\chest\b|strongbox|skippet|caddy|crate|casket|(?<!(cedar|dollhouse|lacquered|rosewood|small|small iron|writer's) )box)
	pause 1
	send accept
	waitfor now holding
	action (waiting) off
...
container_Check1:
	matchre get_For_Disarm \b(coffer|trunk|chest|strongbox|skippet|caddy|crate|casket|(?<!(cedar|lacquered|rosewood|small iron|writer's) )box)\b
	#matchre armor_check1 ^There is nothing in there.
	match container_Check2 Encumbrance
	put rummage in my %container1;enc
	matchwait

container_Check2:
		matchre get_For_Disarm \b(coffer|trunk|chest|strongbox|skippet|caddy|crate|casket|(?<!(cedar|lacquered|rosewood|small iron|writer's) )box)\b
		match armor_Check1 Encumbrance
	put rummage in my %container2;enc
	matchwait
...
disarm_ID:
	var LAST disarm_ID
		...
		#matchre return \b(coffer|trunk|chest|strongbox|skippet|caddy|crate|casket|(?<!(cedar|lacquered|rosewood|small iron|writer's) )box)\b
	put disarm ID
	matchwait
...
armor_Check1:
	...
	put rummage in my %armor.stow;enc
	matchwait

armor_Check2:
		matchre wear_Armor %armor.types
		match done Encumbrance
	put rummage in my %armor.stow2;enc
	matchwait
