...
#% example : Bow|Crossbow|Offhand_Weapon|Polearms|Slings|Small_Edged|Staves|Twohanded_Blunt|Small_Blunt|Large_Blunt|Brawling|Light_Thrown|Targeted_Magic|Large_Edged|Heavy_Thrown|Twohanded_Edged
     ...
     var smalledge tanto 
     var largeedge tachi
     var twohandededge nodachi
     var edge-maneuver none
     var smallblunt bola 
     var largeblunt throwing hammer 
     var twohandedblunt riste 
     var blunt-maneuver none
     var staves nightstick
     var staves-maneuver none
     var polearms spear
     var polearms-maneuver none
     var crossbow pelletbow 
     ...
     var targetmagic on
     var TM-spell-1 STS 10
     var TM-spell-2 CAC 15
     var TM-spell-3 none
     var TM-weapon none
...
#% Debil cast can be set to area, creature, a specific command like "malediction offense" or none for general debil casts.
     ...
     var debilitation on
     var debil-spell swarm 25
     var debil-cast none
     var debil-tatt none
     var debil-tatt-harness 0
...
#% These are 'if not mind locked by this many attacks move on to next skill' defaults.
     ...
     var smalledge-attack-limit 100
     var largeedge-attack-limit 100
     var twohandededge-attack-limit 100
     var smallblunt-attack-limit 100
     var largeblunt-attack-limit 100
     var twohandedblunt-attack-limit 100
     var staves-attack-limit 100
     var polearms-attack-limit 100
     var crossbow-attack-limit 100
     var bow-attack-limit 100
     var slings-attack-limit 100
     var lightthrown-attack-limit 100
     var heavythrown-attack-limit 100
     var offhand-attack-limit 100
     var brawl-attack-limit 120
     var TM-attack-limit 100
...
#% LOOT
     ...
     var box-holders duffel.bag 
     var box-limit 15

#% STANCES  EVA PARRY SHIELD ATTACK
     var change-stance on
     var smalledge-stance 89 98 0 100
     var largeedge-stance 89 98 0 100
     var twohandededge-stance 89 98 0 100
     var smallblunt-stance 89 98 0 100
     var largeblunt-stance 89 98 0 100
     var twohandedblunt-stance 89 98 0 100
     var staves-stance 89 98 0 100
     var polearms-stance 89 98 0 100
     var crossbow-stance 89 0 98 100
     var bow-stance 89 0 98 100
     var slings-stance 89 0 98 100
     var lightthrown-stance 89 0 98 100
     var heavythrown-stance 89 0 98 100
     var offhand-stance 89 0 98 100
     var brawl-stance 89 0 98 100
     var TM-stance 89 0 98 100
     var backstab-stance 100 0 80 100
...
#% SPELL BUFFS
     var buff-spell-1 maf 75
     var buff-camb-1 none
     var buff-spell-2 sott 40
     var buff-camb-2 40
     var buff-spell-3 oath 30
     var buff-camb-3 30
     var buff-spell-4 cotc 35
     var buff-camb-4 35
     var buff-spell-5 stw 40
     var buff-camb-5 40
     var buff-spell-6 none
     var buff-camb-6 none
     var buff-spell-7 none
     var buff-camb-7 25
     var buff-spell-8 em 30
     var buff-camb-8 24
     var buff-spell-9 inst 30
     var buff-camb-9 30
     var buff-spell-10 none
     var buff-camb-10 none
     var minimum-attunement 40
     var camb-item watersilk bag
...
#% CONTAINERS
     ...
     var sheath-1-weapons tanto|tachi|nodachi|spear|staff sling|bola|nightstick|shortbow|riste|sabre
     var sheath-2-name rucksack
     var sheath-2-weapons throwing.hammer|pelletbow
     var sheath-3-name ruckasck
     var sheath-3-weapons none
     var worn-weapons forester's bow|quarterstaff
...
put info
     ...
     if matchre("%offhandmelee","%offhand-type") then var offhand-type melee
...
put swap my $righthandnoun
     ...
     matchwait
swap-sword.d:
     if "%swappietype" = "medium blunt" then if "%skill" = "small edge" then goto swap-sword
     if "%swappietype" = "medium edge" then if "%skill" = "small blunt" then goto swap-sword
...
debil:
     ...
     if "%debil-tatt" = "on" then put invoke tattoo 
     if "%debil-tatt" != "on" then gosub verb prep %debil-spell
     ...
     if "%debil-tatt" = "on" then if "%debil-tatt-harness" != "none" then put harness %debil-tatt-harness 
...
echo %skip-dissect
echo %skincomplete
if (matchre("%critname", "(%skinnables)") && matchre("%critname", "(%dissectable)") && ($Skinning.LearningRate < 26) && matchre(%skip-dissect, (0|1))) then
{  
		put echo SKINNING
             var reskin 0
...
if (($Skinning.LearningRate > 25) && matchre("%critname", "(%dissectable)") && (%skip-dissect = 0)) then
{
	put echo DISSECTING
        var skip-dissect 1
	match skip-skin successfully
	match skip-skin succeed
	match reskin What exactly
	match reskin would not be useful
	match reskin waste of time
	match reskin Roundtime
	put diss
	matchwait

}

if (matchre("%critname", "(%dissectable)") && !matchre("%critname", "(%skinnables)") && (%skip-dissect = 0)) then
{
	put echo DISSECTING
        var skip-dissect 1
	match skip-skin successfully
	match skip-skin succeed
	match skip-skin What exactly
	match skip-skin would not be useful
	match skip-skin waste of time
	match reskin Roundtime
	put diss
	matchwait
}

if (matchre("%critname", "(%skinnables)") && (%skincomplete = 0) && matchre(%skip-dissect, (0|1))) then
{
		put echo SKINNING2
             var reskin 0
    	     if "%arrange-option" != "none" then if "%arrange-option" != "off" then var arrangeverb %critname for %arrange-option
...
prep-buff.lost:
     ...
     if "%spellname" = "mf" then gosub verb remove staff
     if "%spellname" = "pom" then gosub verb remove staff
     if "%spellname" = "blood" then gosub stow-wear
     if "%spellname" = "blood" then gosub verb remove charm 
     ...
     if "$preparedspell" = "Bloodthorns" then gosub verb invoke charm 
...
cast-buff:
     ...
     if "%spellname" = "mf" then gosub verb wear staff
     if "%spellname" = "pom" then gosub verb wear staff
     if "%spellname" = "blood" then gosub verb wear charm
     if "%spellname" = "blood" then gosub prep-get
     gosub set-dur
     return
setspell:
     var spellname $1
...
prep-get:
     if "%skill" = "small edge" then put get %smalledge   
     if "%skill" = "large edge" then put get %largeedge   
     if "%skill" = "2HE" then put get %twohandededge   
     if "%skill" = "small blunt" then put get %smallblunt   
     if "%skill" = "large blunt" then put get %largeblunt    
     if "%skill" = "2HB" then put get %twohandedblunt   
     if "%skill" = "staves" then put remove %staves  
     if "%skill" = "bow" then  put remove %bow
     if "%skill" = "crossbow" then  put get %crossbow  
     if "%skill" = "slings" then  put get %slings  
     if "%skill" = "HT" then  put get %heavythrown  
     if "%skill" = "LT" then  put get %lightthrown  
     if "%skill" = "offhand" then  put get %offhand  
     if "%skill" = "offhand" then put swap
     return
...
set-exptracker:
     ...
     if "%skill" = "offhand" then if $Offhand_Weapon.LearningRate > %exp-limit then var LOCKEDUP 1
     if "%skill" = "brawl" then if $Brawling.LearningRate > %exp-limit then var LOCKEDUP 1
     if "%skill" = "TM" then if $Targeted_Magic.LearningRate > %exp-limit then var LOCKEDUP 1
     return
