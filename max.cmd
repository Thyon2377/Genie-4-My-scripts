...
action var cambrinthfull yes; var currentmana $maxcambrinth.capacity when only absorbing part of the energy while the rest dissipates harmlessly\.$|^The.+is already holding as much power as you could possibly charge it with\.$
...
if %manaadd <= $maxcambrinth.capacity then goto cambrinthonly
if %manaadd > $maxcambrinth.capacity then goto cambrinthharness
...
send charge my $maxcambrinth %charge
wait
math currentmana add %charge
pause .5
if %currentmana >= $maxcambrinth.capacity then
if %currentmana >= $maxcambrinth.capacity then var currentmana $maxcambrinth.capacity
...
{
var currentmana $maxcambrinth.capacity
goto cast
}
...
cambrinthharness:
evalmath charge round(($maxcambrinth.capacity/3),0)
...
harness:
evalmath harness round(((%manaadd - $maxcambrinth.capacity)/3),0)
